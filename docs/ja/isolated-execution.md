...
| check id | 検出する設定ミス |
|---|---|
| `same_host` | 対象サーバーがハブと同一ホスト/同一ファイルシステムである（判定表は下記） |
...
`same_host` は以下の順で評価され、**pass の根拠はカナリアの実測のみ**です。boot_id の
不一致や hostname/uid の不一致は pass の根拠に使いません（どちらも仮想化・偽装されうる
ため）— boot_id の一致と hostname+uid の一致は fail 方向にのみ働きます。

1. カナリアファイル（ハブのデータディレクトリ内の非秘密マーカーファイル）が読み取れて
   内容も一致した → **fail**（同一ファイルシステムを共有している）
2. ハブ・対象サーバー双方の boot_id（カーネルの起動ID、厳密な UUID 形式で検証・小文字に
   正規化した上で比較）が取得でき、かつ一致した → **fail**（同一カーネル＝同一ホスト）
3. カナリアの「不在」が信頼できる測定として得られた（プローブが完走して absent を返し、
   かつ往復後もハブ側のカナリアが同一パス・同一内容で健在 — ローテーション競合の防御）
   → **pass**。boot_id の取得可否は問いません（macOS ハブのように boot_id が常に取得
   できない環境でもこの分岐で pass に到達できます）
4. hostname と uid が両方とも一致した → **fail**（同一ホストの強い示唆）。これは
   **カナリアが測定できなかった場合の補助**です。hostname の再利用は別マシン間でも
   起こり得るうえ uid 1000 はほぼ全ての Linux で既定値のため、直接の実測である
   カナリアが「到達不能」を示している場合は、その偶然の一致で棄却しません
5. 上記のいずれにも当てはまらない（カナリア測定不能、往復中のローテーション検出、
   ハブ側にカナリアが無い、など） → **unknown**

**この判定の限界**: pass はカナリアによる「ハブのデータディレクトリに到達できない」という
実測に基づくものであり、別ホストであることの証明ではありません。同一ホスト上でファイル
システム名前空間が分離されたコンテナは、boot_id を仮想化していれば pass に到達しえます。
ただしその構成でもハブのデータには到達できないため、層2（資格情報を配らない）の前提は
成立します。敵対的ホストは自己申告を偽装できる点は3節のドクトリンのとおりです。
