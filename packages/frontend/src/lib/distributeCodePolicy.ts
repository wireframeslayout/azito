 ...
 * have saved in an earlier, non-isolated session. The distribute-code
 * toggle (components/settings/EnvironmentFormFields.tsx) and the save
 * request built in `ProjectSettings.tsx` both need to agree with that
 * backend behavior; these pure predicates are the single place that encodes
 * it, so the call sites (and their tests) can't silently drift apart.
 */
