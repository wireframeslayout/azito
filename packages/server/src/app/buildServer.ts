...
export async function buildServer(app: FastifyInstance, wiring: Wiring, port: number): Promise<ServerHandles> {
  ...
  await app.register(unitsRoutes, { unitRepo, taskRepo, logRepo, executeTaskUseCase, projectRepo, projectServerRepo, serverRepo, windowRepo, respawnService: windowRespawnService, sidekickLoader: sidekickPackageLoader, unitTypeLoader, taskRestoreService });
