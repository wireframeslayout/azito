...
// Migrations 001->061 must be replayed against a fresh in-memory DB before 062 can run.
...
function getReports(db: Database.Database, name: string): { isolation_report: string | null; isolation_cleanup_report: string | null } {
  return db.prepare('SELECT isolation_report, isolation_cleanup_report FROM servers WHERE name = ?').get(name) as {
    isolation_report: string | null;
    isolation_cleanup_report: string | null;
  };
}

describe('migration 062: isolation_report_split', () => {
  it('adds isolation_cleanup_report as nullable, NULL for existing rows', () => {
    const db = buildSeededDb();
    m062.up(db);

    const row = getReports(db, 'srv-agent');
    expect(row.isolation_cleanup_report).toBeNull();
  });

  it('moves an existing kind:"cleanup" isolation_report into isolation_cleanup_report and nulls the old column', () => {
    const db = buildSeededDb();
    const cleanupReport = JSON.stringify({ kind: 'cleanup', cleanup: 'done', at: '2026-08-15T00:00:00Z' });
    db.prepare('UPDATE servers SET isolation_report = ? WHERE name = ?').run(cleanupReport, 'srv-agent');

    m062.up(db);

    const row = getReports(db, 'srv-agent');
    expect(row.isolation_report).toBeNull();
    expect(JSON.parse(row.isolation_cleanup_report as string)).toEqual({ kind: 'cleanup', cleanup: 'done', at: '2026-08-15T00:00:00Z' });
  });

  it('moves the pending-marker cleanup report (also kind:"cleanup") into isolation_cleanup_report', () => {
    const db = buildSeededDb();
    const pending = JSON.stringify({ kind: 'cleanup', cleanup: 'pending' });
    db.prepare('UPDATE servers SET isolation_report = ? WHERE name = ?').run(pending, 'srv-agent');
  ...
  it('leaves a kind:"verification" isolation_report in place (verification column, untouched)', () => {
    const db = buildSeededDb();
    const verificationReport = JSON.stringify({ kind: 'verification', verified: true, checks: [], probedAt: '2026-08-15T00:00:00Z' });
    db.prepare('UPDATE servers SET isolation_report = ? WHERE name = ?').run(verificationReport, 'srv-agent');

    m062.up(db);

    const row = getReports(db, 'srv-agent');
    expect(JSON.parse(row.isolation_report as string)).toEqual({ kind: 'verification', verified: true, checks: [], probedAt: '2026-08-15T00:00:00Z' });
    expect(row.isolation_cleanup_report).toBeNull();
  });

  it('leaves an unparseable isolation_report untouched rather than guessing its kind', () => {
    const db = buildSeededDb();
    db.prepare('UPDATE servers SET isolation_report = ? WHERE name = ?').run('{ not valid json', 'srv-agent');

    m062.up(db);

    const row = getReports(db, 'srv-agent');
    expect(row.isolation_report).toBe('{ not valid json');
    expect(row.isolation_cleanup_report).toBeNull();
  });

  it('a row with isolation_report already NULL is unaffected', () => {
    const db = buildSeededDb();
    m062.up(db);

    const row = getReports(db, 'srv-local');
    expect(row.isolation_report).toBeNull();
    expect(row.isolation_cleanup_report).toBeNull();
  });
});
