...
// Migrations 001->060 must be replayed against a fresh in-memory DB before 061 can run.
...
describe('migration 061: isolation_profile', () => {
  it('adds isolation_intent defaulting to 0 (not isolated) for existing rows', () => {
    const db = buildSeededDb();
    m061.up(db);

    const row = db.prepare('SELECT isolation_intent FROM servers WHERE name = ?').get('srv-agent') as Record<string, unknown>;
    expect(row.isolation_intent).toBe(0);
  });

  it('accepts isolation_intent = 1 for an agent server', () => {
    const db = buildSeededDb();
    m061.up(db);

    db.prepare('UPDATE servers SET isolation_intent = 1 WHERE name = ?').run('srv-agent');
    const row = db.prepare('SELECT isolation_intent FROM servers WHERE name = ?').get('srv-agent') as Record<string, unknown>;
    expect(row.isolation_intent).toBe(1);
  });

  it('adds isolation_verified_at and isolation_report as nullable, NULL for existing rows', () => {
    const db = buildSeededDb();
    m061.up(db);

    const row = db.prepare('SELECT isolation_verified_at, isolation_report FROM servers WHERE name = ?').get('srv-agent') as Record<string, unknown>;
    expect(row.isolation_verified_at).toBeNull();
    expect(row.isolation_report).toBeNull();
  });

  it('accepts an explicit isolation_verified_at/isolation_report pair (isolation doctor write shape)', () => {
    const db = buildSeededDb();
    m061.up(db);

    const report = JSON.stringify({ checkedAt: '2026-08-15T00:00:00Z', findings: [] });
    db.prepare('UPDATE servers SET isolation_verified_at = ?, isolation_report = ? WHERE name = ?').run(
      '2026-08-15T00:00:00Z',
      report,
      'srv-agent',
    );
    const row = db.prepare('SELECT isolation_verified_at, isolation_report FROM servers WHERE name = ?').get('srv-agent') as Record<string, unknown>;
    expect(row.isolation_verified_at).toBe('2026-08-15T00:00:00Z');
    expect(JSON.parse(row.isolation_report as string)).toEqual({ checkedAt: '2026-08-15T00:00:00Z', findings: [] });
  });

  it('a local server also gets the new columns (defaulting to not-isolated)', () => {
    const db = buildSeededDb();
    m061.up(db);

    const row = db.prepare('SELECT isolation_intent FROM servers WHERE name = ?').get('srv-local') as Record<string, unknown>;
    expect(row.isolation_intent).toBe(0);
  });
});
