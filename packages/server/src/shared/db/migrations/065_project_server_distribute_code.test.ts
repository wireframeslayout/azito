...
describe('migration 065: project_server_distribute_code', () => {
  it('adds distribute_code defaulting to 0 (off) for existing rows', () => {
    ...
    const row = db.prepare(`SELECT distribute_code FROM project_servers WHERE project_id = 1 AND server_name = 'srv-agent'`).get() as { distribute_code: number };
    expect(row.distribute_code).toBe(0);
  });

  it('accepts distribute_code = 1 after the migration runs', () => {
    const db = buildSeededDb();
    db.prepare(`INSERT INTO project_servers (project_id, server_name, tmux_session) VALUES (1, 'srv-agent', 'azito')`).run();
    m065.up(db);

    db.prepare(`UPDATE project_servers SET distribute_code = 1 WHERE project_id = 1 AND server_name = 'srv-agent'`).run();

    const row = db.prepare(`SELECT distribute_code FROM project_servers WHERE project_id = 1 AND server_name = 'srv-agent'`).get() as { distribute_code: number };
    expect(row.distribute_code).toBe(1);
  });

  it('a row created for a local server also gets the new column, defaulting to off', () => {
    const db = buildSeededDb();
    db.prepare(`INSERT INTO project_servers (project_id, server_name, tmux_session) VALUES (1, 'srv-local', 'azito')`).run();
    m065.up(db);

    const row = db.prepare(`SELECT distribute_code FROM project_servers WHERE project_id = 1 AND server_name = 'srv-local'`).get() as { distribute_code: number };
    expect(row.distribute_code).toBe(0);
  });
});
