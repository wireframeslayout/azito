...
describe('migration 072: fix herdr/zellij mux_ref kind', () => {
  ...
  it('corrects kind from tmux to herdr for herdr server windows', () => {
  ...
  it('corrects kind from tmux to zellij for zellij server windows', () => {
  ...
  it('handles mixed servers in one run', () => {
    insertServer(db, 'herdr-srv', 'herdr');
    insertServer(db, 'zellij-srv', 'zellij');
    insertServer(db, 'tmux-srv', 'system');
    insertWindow(db, 'herdr-srv', 'azito:h1', '{"kind":"tmux","workspace":"azito","window":"h1"}');
    insertWindow(db, 'zellij-srv', 'azito:z1', '{"kind":"tmux","workspace":"azito","window":"z1"}');
    insertWindow(db, 'tmux-srv', 'azito:t1', '{"kind":"tmux","workspace":"azito","window":"t1"}');

    db.transaction(() => m072.up(db))();

    const herdr = db.prepare('SELECT mux_ref FROM windows WHERE server_name = ?').get('herdr-srv') as { mux_ref: string };
    const zellij = db.prepare('SELECT mux_ref FROM windows WHERE server_name = ?').get('zellij-srv') as { mux_ref: string };
    const tmux = db.prepare('SELECT mux_ref FROM windows WHERE server_name = ?').get('tmux-srv') as { mux_ref: string };

    expect(herdr.mux_ref).toBe('{"kind":"herdr","workspace":"azito","window":"h1"}');
    expect(zellij.mux_ref).toBe('{"kind":"zellij","workspace":"azito","window":"z1"}');
    expect(tmux.mux_ref).toBe('{"kind":"tmux","workspace":"azito","window":"t1"}');
  });
