...
export class SqliteProjectServerRepository implements IProjectServerRepository {
  ...
  constructor(private db: SqliteDatabase) {
    ...
    this.upsertStmt = db.prepare(`
      INSERT INTO project_servers (project_id, server_name, working_directory, branch, tmux_session)
      VALUES (?, ?, ?, ?, ?)
      ON CONFLICT(project_id, server_name)
      DO UPDATE SET working_directory = excluded.working_directory, branch = excluded.branch, tmux_session = excluded.tmux_session
    `);
