...
function makeOpts(taskOverrides?: Partial<Task>): TasksRouteOptions {
  ...
  return {
    ...
    muxDriverRegistry: { resolve: () => ({ kind: 'tmux', closeWindow: vi.fn(async () => ({ stdout: '', stderr: '', code: 0 })) }) } as any,
  };
}
