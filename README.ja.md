...
### マルチサーバー対応
Tailscale によるプライベートネットワーク経由で複数の開発サーバーに接続。ローカル・SSH・Agent の3種類のサーバータイプをサポートし、Agent タイプではリモートサーバーへの自動インストール・自動アップデートに対応しています。
...
### PWA 対応
プッシュ通知（タスク完了・失敗時）、ホーム画面への追加、オフラインサポートに対応。スマートフォンからでもネイティブアプリに近い操作感で利用できます。
...
### tmux 実行系統の選択
サーバーごとに、システムの tmux と AZITO 管理の tmux（SHA256 検証付きで自動ダウンロードされる静的バイナリ）を選択できます。推奨 tmux 設定はサーバー設定画面から適用できます。
...
Browser (React 19 + Vite)
  ...
  └── Fastify Server
        ...
        │     ├── tmux/, servers/          # 基盤層: tmux クライアント、サーバートランスポート（Local / SSH / Agent）
...
| SSH | Tailscale SSH (`none` auth) | リモート開発サーバー |
| Agent | AZITO Agent (HTTP) | リモートサーバー（自動インストール対応） |
...
1. **サーバーの確認** -- デフォルトで `local` サーバーが登録済み。SSH / Agent サーバーは Settings から追加
